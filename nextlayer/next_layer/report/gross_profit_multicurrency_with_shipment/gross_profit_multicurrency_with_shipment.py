...
from nextlayer.next_layer.api.currency_converter import convert
...
class GrossProfitGenerator:
    def __init__(self, filters=None):
        ...
        # ─── Sales Shipment Cost ───────────────────────────────────────────────
        # Load shipment costs only when the checkbox is ticked
        self.shipment_costs = {}
        if self.filters.get("include_sales_shipment_cost"):
            self.load_sales_shipment_costs()
        # ──────────────────────────────────────────────────────────────────────

        self.process()

    # ──────────────────────────────────────────────────────────────────────────
    # Sales Shipment Cost helpers
    # ──────────────────────────────────────────────────────────────────────────

    def load_sales_shipment_costs(self):
        """
        Fetches all submitted Sales Shipment Cost documents linked to the
        invoices in si_list and builds a lookup:

            self.shipment_costs[(invoice, item_code, qty)] = applicable_charges
                                                             (in company currency)

        Keyed by (invoice, item_code, qty) so that two lines of the same
        item_code on the same invoice are distinguished by their quantity.
        Multiple SSC docs targeting the same (invoice, item_code, qty) are
        summed together.

        applicable_charges on the SSC Item table are stored in the SSC document
        currency (typically USD).  They are converted to company currency here
        so that when added to buying_amount (which is always in company currency)
        both values are on the same currency base.  All presentation-currency
        conversion then happens later in convert_currency_columns() as normal.
        """
        if not self.si_list:
            return

        invoice_names = list({row.parent for row in self.si_list if row.parent})
        ...
        conversion_date = self.filters.get("to_date") or frappe.utils.today()

        # applicable_charges are always stored in USD on the SSC document
        ssc_currency = "USD"

        rows = frappe.db.sql(
            """
            SELECT
                ssc_pr.receipt_document          AS invoice,
                ssc_item.item_code               AS item_code,
                ssc_item.qty                     AS qty,
                SUM(ssc_item.applicable_charges) AS shipment_amount
            FROM
                `tabSales Shipment Cost` ssc
                INNER JOIN `tabLanded Cost Sales Invoice` ssc_pr
                    ON ssc_pr.parent = ssc.name
                INNER JOIN `tabSales Shipment Cost Item` ssc_item
                    ON ssc_item.parent = ssc.name
            WHERE
                ssc.docstatus = 1
                AND ssc_pr.receipt_document IN %(invoices)s
            GROUP BY
                ssc_pr.receipt_document,
                ssc_item.item_code,
                ssc_item.qty
            """,
        ...
        for r in rows:
            # Convert USD → company currency before storing.
            # If company currency is also USD, convert() returns the value unchanged.
            # frappe.throw(str(r.shipment_amount))
            amount_in_company_currency = convert(
                flt(r.shipment_amount),
                
                company_currency,
                ssc_currency,
                conversion_date,
            )
            # frappe.throw(str(amount_in_company_currency))
            key = (r.invoice, r.item_code, flt(r.qty))
            self.shipment_costs[key] = (
                self.shipment_costs.get(key, 0.0) + amount_in_company_currency
            )

    def get_shipment_cost_for_row(self, row):
        """
        Returns the applicable_charges from the Sales Shipment Cost that should
        be added to the buying amount for this row.

        Lookup uses the composite key (invoice, item_code, qty) — the same
        three fields grouped on in load_sales_shipment_costs — so that two
        lines of the same item with different quantities resolve independently.
        """
        if not self.shipment_costs:
            return 0.0
        ...
        if not invoice or not item_code:
            return 0.0

        return self.shipment_costs.get((invoice, item_code, qty), 0.0)
    ...
    def process(self):
        ...
        for row in reversed(self.si_list):
            ...
            # ── Add Sales Shipment Cost to buying amount ───────────────────
            # Only applied to item-level rows (indent 1.0) or non-invoice grouping.
            # Header rows (indent 0.0) are re-calculated below from their children.
            if self.filters.get("include_sales_shipment_cost"):
                is_item_row = (
                    not grouped_by_invoice          # non-invoice grouping: always an item row
                    or row.get("indent") == 1.0     # invoice grouping: child item row
                )
                if is_item_row:
                    shipment_cost = self.get_shipment_cost_for_row(row)
                    row.buying_amount = flt(
                        row.buying_amount + shipment_cost, self.currency_precision
                    )
            # ──────────────────────────────────────────────────────────────
